...
def OpenXOSC():
    ...
    if scen_file:
        ...
        if debug_flag == 1:
            print("LIST: imported_param_list")
...
def ImportedLBSelect(event):
    ...
    if selection:
        ...
        if imported_param_list[index].type == "TEXT":
            rb_type_select.set(1)
        elif imported_param_list[index].type == "SET":
            rb_type_select.set(2)
...
def AddItem():
    ...
    if rb_type_select.get() == 1:
        ...
        mod_param = Parameter(param_to_add.name, [str(value)], "TEXT", 1)
...
def GenerateFiles():
    ...
    file_name = getFileName(base_file_name)
    ...
    try:
        ...
        if debug_flag == 1:
            print("ERROR: {} directory exists.".format(base_file_name))

    inner_path = os.path.join(path, file_name)
    ...
    if debug_flag == 1:
        print("Base path: {}\nFolder name: {}\n Directory created: {}\n".format(base_path, file_name, path))
    ...
    for item in final_list:
        ...
        string = "{BP}\{BFN}_{i}.xosc".format(BP = inner_path, BFN = base_file_name, i = i)

        shutil.copy(scen_name.get(), string)
        ...
        new_string = "{BP}\ID-{}_{BFN}_{x}_{y}_{z}.xosc".format(batch_number, BP = inner_path, BFN = base_file_name, x = les, y = los, z = lrdte)
...
def RBChoice():
    ...
    if rb_type_select.get() == 2:
        choice_msg.set("Please format your set like so: 1 2 3 4 5")

    if rb_type_select.get() == 3:
        choice_msg.set("Please enter as: Lower Step Upper (ex. 10 5 20)")
